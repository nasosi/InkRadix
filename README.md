...
2. Place the files ```InkRadix.inx``` and ```InkRadix.py``` in the following user directory: ```%APPDATA%\inkscape\extensions\nasos.inkradix```. Note: You can copy and paste ``` %APPDATA%\inkscape\extensions```,  directly into the Windows File Explorer address bar. It will resolve to a folder similar to: ```C:\Users\<YourUsername>\AppData\Roaming\inkscape\extensions```. Then create the folder ```nasos.inkradix```.
3. (Re)start Inkscape to load the extension.
# Usage
